...
### ... Input files ----
pm <- read.delim('./DATA/ANALYSIS_DATA/2way__OG/2wOG_PERM_analysis_mf1-cf10_Tissue-Stage-PM.tsv', sep = '\t')
pwowm <- read.delim('./DATA/ANALYSIS_DATA/2way__OG/2wOG_PERM_analysis_mf1-cf10_Tissue-Stage-PWOWM.tsv', sep = '\t')
...
### ... Saving files ----
write.table(ALL,
            sprintf('./DATA/ANALYSIS_DATA/2way__OG/2wOG_%s_gene-perturbation_%s_%s.tsv',
                    'PERM', FREQ, SPLITMOD),
