...
FDR_conversion_table <- read.delim(sprintf("./DATA/ANALYSIS_DATA/2way__T/2wT_FDR-conversion-table_%s_%s.tsv",FREQ,SPLITMOD),
                                   sep="\t", header=T)
...
if (!file.exists('./DATA/ANALYSIS_DATA/2way__T')){
  dir.create('./DATA/ANALYSIS_DATA/2way__T')
}
setwd('./DATA/ANALYSIS_DATA/2way__T')
