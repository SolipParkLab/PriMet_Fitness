...
# Model inputs
model_inputs_df <- read.delim('./DATA/GLM_INPUTS/2way_Treatment/2way_Treatment__inputs_mf1-cf10_Tissue-Stage-PM.tsv', sep = '\t')
...
# Saving
write.table(full_output, './DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_TvsNT_Tarone-output.tsv',
...
saveRDS(false_output_concat, './DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_TvsNT_Tarone-randomization-output.RDS')
...
# Saving file
write.table(tarone_res, './DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_TvsNT_Tarone-output-adj-pval.tsv',
            sep = '\t', row.names = F, quote = F)
