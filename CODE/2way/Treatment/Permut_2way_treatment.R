...
saveRDS(permutation_matrixes_alter$perm,
        sprintf("./DATA/PROCESSED_DATA/treatment_matrices/2way_Treatment-PERMUT_matrixes_%s%s.RDS",binary_mat_name,alter))


# When all of them have been done
files <- list.files(path="./DATA/PROCESSED_DATA/treatment_matrices/",
                    pattern="2way_Treatment-PERMUT_matrixes")
...
saveRDS(permuted_outputs,
        sprintf("./DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment-PERMUT_matrix_outputs_%s.RDS", FREQ, SPLITMOD, permuted_matrix_name))
##  For loading 100 permuted files into a single file
files <- list.files(path="./DATA/ANALYSIS_DATA/2way_Treatment/",
                    pattern="2way_Treatment-PERMUT_*")
model_names <- str_replace_all(str_replace_all(files,"2way_Treatment-PERMUT_matrix_outputs_",""),".RDS","")
permuted_results <- lapply(files,function(name){
  return(readRDS(sprintf("./DATA/ANALYSIS_DATA/2way_Treatment/%s",name)))
})
...
saveRDS(permuted_results,
        "./DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_permutation-outputs.RDS")



#### 3. COUNTING SIGNIFICANTS #####
### ... Uploading files ----
permuted_results <- readRDS("./DATA/ANALYSIS_DATA/2way_Treatment_permutation-outputs.RDS")
...
write.table(significant_tables_counts,
            sprintf("./DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_permutation_sig-counts_%s.tsv",args[2]),
...
##  For loading 100 permuted files into a single file
files <- list.files(path="./DATA/ANALYSIS_DATA/2way_Treatment/",
                    pattern="2way_Treatment_permutation_sig-counts_counts*")
# files <- files[1:50] # I think this has to be commented for 1 to 100
res <- lapply(files,function(name){
  count_file <- read.delim(sprintf("./DATA/ANALYSIS_DATA/2way_Treatment/%s", name),
                           header=T, sep="\t")
  return(count_file)
})
...
saveRDS(resu,
        sprintf("./DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_bindrows-permutation_sig-counts_%s_%s.RDS", FREQ, SPLITMOD))
...
write.table(count_real_significants, sprintf("./DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_permutation_real-counts_%s_%s.tsv", FREQ, SPLITMOD),
            sep="\t", quote=F, row.names=F)


### ... Merging count tables (permutation + real results) ----
## Uploading counts ----
significant_tables_counts <- readRDS(sprintf("./DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_bindrows-permutation_sig-counts_%s_%s.RDS", FREQ, SPLITMOD))
count_real_significants <- read.delim(sprintf("./DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_permutation_real-counts_%s_%s.tsv", FREQ, SPLITMOD),
                                        sep="\t", header=T)
...
write.table(count_real_significants,
            sprintf("./DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_permutation_summarised-real-counts_%s_%s.tsv",FREQ,SPLITMOD,FREQ,SPLITMOD),
            sep="\t", quote=F, row.names=F)
...
write.table(permuted_significants_total,
            sprintf("./DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_bindrows-permutation_FDR-table_%s_%s.tsv", FREQ, SPLITMOD),
            sep="\t", quote=F, row.names=F)



#### 4. PLOTTING THINGS #####
permuted_significants_total <- read.delim(sprintf("./DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_bindrows-permutation_FDR-table_%s_%s.tsv", FREQ, SPLITMOD),
                                          sep="\t", header=T)
...
write.table(FDR_conversion_table,
            sprintf("./DATA/ANALYSIS_DATA/2way_Treatment/2way_Treatment_FDR-conversion-table_%s_%s.tsv", FREQ, SPLITMOD),
            sep="\t", quote=F, row.names=F)
