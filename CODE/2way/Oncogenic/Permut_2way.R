...
saveRDS(permuted_outputs,
        sprintf("./DATA/ANALYSIS_DATA/2way/2way_permutation_outputs_%s_%s.RDS",FREQ,SPLITMOD))
...
write.table(count_real_significants, sprintf("./DATA/ANALYSIS_DATA/2way_permutation_real-counts_%s_%s.tsv", FREQ, SPLITMOD),
...
if (!file.exists('./DATA/GLM_OUTPUTS/2way/temp_permutation')){
  dir.create('./DATA/GLM_OUTPUTS/2way/temp_permutation')
}
...
write.table(significant_pairs_table, sprintf("./DATA/ANALYSIS_DATA/2way/temp_permutation/2way_permutation_sig-counts_%s_%s_%s.tsv",
...
count_real_significants <- read.delim(sprintf("./DATA/ANALYSIS_DATA/2way_permutation_real-counts_%s_%s.tsv", FREQ, SPLITMOD),
                                        sep="\t", header=T)
files <- list.files(path = "./DATA/ANALYSIS_DATA/2way/temp_permutation/",
                    pattern = "2way_permutation_sig-counts_*")
...
count_file_list <- lapply(files,function(name){
  count_file <- read.delim(sprintf("./DATA/ANALYSIS_DATA/2way/temp_permutation/%s", name),
                           header=T, sep="\t")
  if(grepl("Tissue",SPLITMOD)){
    if(grepl("PM",SPLITMOD)){
      print("Number of rows should be: 8032000")
...
write.table(count_files, sprintf("./DATA/ANALYSIS_DATA/2way/2way_permutation_all-counts-with-FDR_%s_%s.tsv", FREQ,SPLITMOD),
            sep="\t", quote=F, row.names=F)
saveRDS(count_files, sprintf("./DATA/ANALYSIS_DATA/2way/2way_permutation_all-counts-with-FDR_%s_%s.RDS", FREQ, SPLITMOD))
...
write.table(count_files, sprintf("./DATA/ANALYSIS_DATA/2way/2way_permutation_FDR-table_%s_%s.tsv", FREQ, SPLITMOD),
            sep="\t", quote=F, row.names=F)



#### 5. Getting final FDR conversion table ----
permuted_significants_total <- read.delim(sprintf("./DATA/ANALYSIS_DATA/2way/2way_permutation_FDR-table_%s_%s.tsv", FREQ, SPLITMOD),
                                          sep="\t", header=T)
...
write.table(FDR_conversion_table, sprintf("./DATA/ANALYSIS_DATA/2way/2way_FDR-conversion-table_%s_%s.tsv", FREQ, SPLITMOD),
            sep="\t", quote=F, row.names=F)
