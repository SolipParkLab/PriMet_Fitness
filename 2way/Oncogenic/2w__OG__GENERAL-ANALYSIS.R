...
### ... Uploading files ----
model_results <- readRDS(sprintf("./DATA/MODEL_OUTPUTS/2way__OG/2w__OG__outputs_%s_%s.RDS",FREQ,SPLITMOD))
model_inputs <- read.delim(sprintf("./DATA/MODEL_INPUTS/2way__OG/2w__OG__inputs_%s_%s.tsv",FREQ,SPLITMOD),
                           sep="\t",
                           header=T)
